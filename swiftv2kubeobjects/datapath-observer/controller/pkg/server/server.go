...
func (s *Server) Start(addr string) error {
	...
	mux.HandleFunc("/api/v1/pod-health", s.handlePodHealth)
...
func (s *Server) handlePodHealth(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	...
	result, err := s.Calculator.GetPodHealth(r.Context(), namespace, labels, topN)
	if err != nil {
