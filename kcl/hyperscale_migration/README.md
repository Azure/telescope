...
2. Add a 700-node `Standard_D8s_v3` pool for KWOK controllers.
3. Create and validate 2,000 simulated KWOK Nodes, each with a dedicated
  controller.
...
```bash
kcl run kcl/hyperscale_migration/ \
  -S output \
  -o kcl/hyperscale_migration/pipeline.yaml
```