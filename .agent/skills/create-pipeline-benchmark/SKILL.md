...
**Identity & placement** (ask in this order)
- **Team / group folder** (`<GROUP>`) — ask this **first**, e.g. `ccp_team`,
  `apiserver_benchmark`. The benchmark folder is nested under it.
- **Benchmark name** (`<NAME>`) — ask this **second**; it becomes the folder
  name under `<GROUP>`, e.g. `hyperscale_pod_scheduling`
