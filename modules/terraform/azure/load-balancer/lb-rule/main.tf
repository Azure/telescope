...
resource "azurerm_lb_rule" "lb-rule" {
  ...
  name                           = var.rule_count == 1 ? "${var.name_prefix}" : "${var.name_prefix}-${count.index + 1}"
  ...
  frontend_ip_configuration_name = "${var.frontend_ip_config_name_prefix}-lb-frontend-ip"
...
resource "azurerm_lb_outbound_rule" "lb-outbound-rule" {
  ...
  frontend_ip_configuration {
    name = "${var.frontend_ip_config_name_prefix}-lb-frontend-ip"
  }
}
