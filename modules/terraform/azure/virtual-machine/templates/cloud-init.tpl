...
runcmd:
  ...
  - bash -c 'cd /tmp && KUBECTL_VERSION=$(curl -sSfL --connect-timeout 10 --max-time 60 --retry 5 --retry-delay 5 https://dl.k8s.io/release/stable.txt) && curl -sSfLO --connect-timeout 10 --max-time 60 --retry 5 --retry-delay 5 "https://dl.k8s.io/release/$KUBECTL_VERSION/bin/linux/amd64/kubectl" && curl -sSfLO --connect-timeout 10 --max-time 60 --retry 5 --retry-delay 5 "https://dl.k8s.io/release/$KUBECTL_VERSION/bin/linux/amd64/kubectl.sha256" && echo "$(cat kubectl.sha256)  kubectl" | sha256sum --check && install -o root -g root -m 0755 kubectl /usr/local/bin/kubectl && rm -f kubectl kubectl.sha256'
