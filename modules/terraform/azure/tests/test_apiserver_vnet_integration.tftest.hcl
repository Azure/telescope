...
# Test 1: Enable API Server VNet Integration with subnet name
# Pipeline: ENABLE_APISERVER_VNET_INTEGRATION=true, API_SERVER_SUBNET_NAME=apiserver-subnet
# Expected: api_server_subnet_id should be calculated and passed to CLI command
run "apiserver_vnet_enabled_with_subnet" {
  ...
  assert {
    condition     = var.json_input["api_server_subnet_name"] == "apiserver-subnet"
    error_message = "json_input: api_server_subnet_name should be 'apiserver-subnet'"
  }
}
...
# Pipeline: ENABLE_APISERVER_VNET_INTEGRATION=true, but API_SERVER_SUBNET_NAME not set
# Expected: api_server_subnet_id should be null, no CLI parameters added
