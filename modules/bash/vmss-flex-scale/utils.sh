...
measure_create_vmss() {
    ...
    if [[ "$cloud" == "aws" ]]; then
        security_group_id=$(aws ec2 describe-security-groups --filters Name=tag:run_id,Values=$run_id Name=tag:Name,Values=$network_security_group --query "SecurityGroups[0].GroupId" --output text)
        subnet_id=$(aws ec2 describe-subnets --filters Name=tag:run_id,Values=$run_id Name=tag:Name,Values=$subnet --query "Subnets[0].SubnetId" --output text)
    fi
