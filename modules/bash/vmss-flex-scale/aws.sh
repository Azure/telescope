...
#   - $4: vpc_id: The VPC identifier where the VM will be created (e.g. vpc-1234567890)
#
# Usage: create_lt <lt_name> <vm_size> <vm_os> <vpc_id>
create_lt() {
    local lt_name=$1
    local vm_size=$2
    local vm_os=$3
    ...
    launch_template_id=$(aws ec2 create-launch-template --launch-template-name $lt_name  \
                            --launch-template-data "{\"ImageId\":\"$vm_os\",\"InstanceType\":\"$vm_size\", \"NetworkInterfaces\":[{\"SubnetId\":[\"$vpc_id\"]}]}" \
