...
DAEMONSETS_PER_NODE_MAP = {"aws": 2, "aks": 6}
MEMORY_SCALE_FACTOR = (
    0.95  # 95% of the total allocatable memory to account for error margin
)
...
def override_config_clusterloader2(
    ...
    print(
        f"Node {node.metadata.name} has allocatable cpu of {allocatable_cpu} and allocatable memory of {allocatable_memory}"
    )
    ...
    print(
        f"Node {node.metadata.name} has cpu value of {cpu_value} and memory value of {memory_value}"
    )

    allocated_cpu, allocated_memory = _get_daemonsets_pods_allocated_resources(
        client, node.metadata.name
    )
    print(
        f"Node {node.metadata.name} has allocated cpu of {allocated_cpu} and allocated memory of {allocated_memory}"
    )
    ...
    print(
        f"CPU request for each pod: {cpu_request}m, memory request for each pod: {memory_request_in_k}K, total pod per node: {pod_count}"
    )

    # Calculate the number of steps to scale up
    steps = node_count // node_per_step
    print(
        f"Scaled enabled: {scale_enabled}, node per step: {node_per_step}, steps: {steps}, scrape kubelets: {scrape_kubelets}"
    )
    ...
    with open(override_file, "w", encoding="utf-8") as file:
        ...
        file.write(
            f"CL2_POD_STARTUP_LATENCY_THRESHOLD: {pod_startup_latency_threshold}\n"
        )
...
def execute_clusterloader2(
    cl2_image, cl2_config_dir, cl2_report_dir, kubeconfig, provider, scrape_kubelets
):
    run_cl2_command(
        kubeconfig,
        cl2_image,
