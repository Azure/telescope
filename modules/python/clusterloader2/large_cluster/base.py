...
class ClusterLoader2Base(ABC):
    ...
    _command_parser: argparse.ArgumentParser
    ...
    def _add_subparser(self, command: str, description: str):
        add_args_method = {
            Command.CONFIGURE.value: self.add_configure_args,
            Command.VALIDATE.value: self.add_validate_args,
            Command.EXECUTE.value: self.add_execute_args,
            Command.COLLECT.value: self.add_collect_args,
        }.get(command)

        if not add_args_method:
            return

        parser = self._command_parser.add_parser(command, help=description)
    ...
    def parse_test_results(self, cl2_report_dir: str) -> tuple[str, list[any]]:
        ...
        details = self.parse_xml_to_json(junit_xml_file, indent=2)
