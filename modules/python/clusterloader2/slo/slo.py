...
def calculate_config(cpu_per_node, node_count, max_pods, provider, service_test, cnp_test, ccnp_test, churn_identities):
    throughput = 100
    ...
    if cnp_test or ccnp_test or churn_identities:
        pods_per_node = max_pods
...
def configure_clusterloader2(
    ...
    throughput, nodes_per_namespace, pods_per_node, cpu_request = calculate_config(cpu_per_node, node_per_step, max_pods, provider, service_test, cnp_test, ccnp_test, churn_identities)
    ...
    with open(override_file, 'w', encoding='utf-8') as file:
        ...
        if ccnp_test:
            ...
            file.write("CL2_GROUP_NAME: cnp-ccnp\n")
        if churn_identities:
            file.write("CL2_CHURN_IDENTITIES: true\n")
        else:
            file.write("CL2_CHURN_IDENTITIES: false\n")
...
def collect_clusterloader2(
    ...
    _, _, pods_per_node, _ = calculate_config(cpu_per_node, node_count, max_pods, provider, service_test, cnp_test, ccnp_test, churn_identities)
...
def main():
    ...
    parser_configure.add_argument("churn_identities", type=str2bool, choices=[True, False], nargs='?', default=False,
                                  help="Whether identities churn is enabled. Must be either True or False")
    ...
    parser_collect.add_argument("ccnp_test", type=str2bool, choices=[True, False], nargs='?', default=False,
                                  help="Whether ccnp test is running. Must be either True or False")
    parser_collect.add_argument("churn_identities", type=str2bool, choices=[True, False], nargs='?', default=False,
                                  help="Whether identities churn is enabled. Must be either True or False")
    ...
    if args.command == "configure":
        configure_clusterloader2(args.cpu_per_node, args.node_count, args.node_per_step, args.max_pods,
                                 ...
                                 args.service_test, args.cnp_test, args.ccnp_test, args.num_cnps, args.num_ccnps, args.churn_identities, args.dualstack, args.cl2_override_file)
        ...
        collect_clusterloader2(args.cpu_per_node, args.node_count, args.max_pods, args.repeats,
                               ...
                               args.service_test, args.cnp_test, args.ccnp_test, args.churn_identities,
