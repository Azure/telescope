...
def warmup_deployment_for_karpeneter(
    cl2_config_dir, warmup_deployment_template="warmup_deployment.yaml"
):
...
def calculate_cpu_request_for_clusterloader2(
    ...
    try:
        nodes = client.wait_for_nodes_ready(
            1, timeout, label_selector=node_label_selector
        )
        if len(nodes) == 0:
            raise Exception(
                f"No nodes found with label selector: {node_label_selector}"
            )
    except Exception as e:
    ...
    allocated_cpu, _ = client.get_daemonsets_pods_allocated_resources(
        "kube-system", node.metadata.name
    )
    ...
    if warmup_deployment in ["true", "True"]:
        ...
        cleanup_warmup_deployment_for_karpeneter(
            cl2_config_dir, warmup_deployment_template
        )
...
def override_config_clusterloader2(
    ...
    cpu_request = calculate_cpu_request_for_clusterloader2(
        node_label_selector,
        node_count,
        pod_count,
        warmup_deployment,
...
def execute_clusterloader2(
    cl2_image, cl2_config_dir, cl2_report_dir, kubeconfig, provider
):
    run_cl2_command(
        kubeconfig, cl2_image, cl2_config_dir, cl2_report_dir, provider, overrides=True
    )
        ...
        for testcase in testsuites[0]["testcases"]:
            ...
            if match:
                ...
                if index not in summary:
                    summary[index] = {"up": {"failures": 0}, "down": {"failures": 0}}
            else:
                continue
            ...
            for test_key, (category, summary_key) in metric_mappings.items():
                if test_key in name:
                    summary[index][category][summary_key] = (
                        -1 if failure else testcase["time"]
                    )
        ...
        for index, inner_dict in summary.items():
            for key, value in inner_dict.items():
                data = {
                    ...
                    "wait_for_50Perc_nodes_seconds": value[
                        "wait_for_50Perc_nodes_seconds"
                    ],
                    "wait_for_70Perc_nodes_seconds": value[
                        "wait_for_70Perc_nodes_seconds"
                    ],
                    "wait_for_90Perc_nodes_seconds": value[
                        "wait_for_90Perc_nodes_seconds"
                    ],
                    "wait_for_99Perc_nodes_seconds": value[
                        "wait_for_99Perc_nodes_seconds"
                    ],
                    "wait_for_pods_seconds": value["wait_for_pods_seconds"],
                    "autoscale_result": (
                        "success" if value["failures"] == 0 else "failure"
                    ),
                }
...
def main():
    ...
    parser_override.add_argument(
        "cpu_per_node", type=int, help="Name of cpu cores per node"
    )
    ...
    parser_override.add_argument(
        "scale_up_timeout", type=str, help="Timeout before failing the scale up test"
    )
    parser_override.add_argument(
        "scale_down_timeout",
        type=str,
        help="Timeout before failing the scale down test",
    )
    parser_override.add_argument(
        "loop_count", type=int, help="Number of times to repeat the test"
    )
    parser_override.add_argument(
        "node_label_selector", type=str, help="Node label selector"
    )
    parser_override.add_argument(
        "node_selector", type=str, help="Node selector for the test pods"
    )
    parser_override.add_argument(
        "cl2_override_file", type=str, help="Path to the overrides of CL2 config file"
    )
    parser_override.add_argument(
        "warmup_deployment", type=str, help="Warmup deployment to get the cpu request"
    )
    parser_override.add_argument(
    ...
    parser_override.add_argument(
        ...
        default="linux",
        help="Operating system type for the node pools",
    )
    parser_override.add_argument(
        "--warmup_deployment_template",
        type=str,
    ...
    parser_override.add_argument(
        ...
        type=str,
        default="",
        help="Path to the CL2 deployment file",
    )
