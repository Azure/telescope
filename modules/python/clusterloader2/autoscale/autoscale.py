...
def _get_daemonsets_pods_allocated_resources(client, node_name):
    pods = client.get_pods_by_namespace(
        "kube-system", field_selector=f"spec.nodeName={node_name}"
    )
    cpu_request = 0
    for pod in pods:
...
def calculate_cpu_request_for_clusterloader2(
    node_label_selector, node_count, pod_count, warmup_deployment
):
    ...
    try:
        while elapsed < timeout:
            ...
            if len(nodes) > 0:
                break
            print(
                f"No nodes found with the label {node_label_selector}. Retrying in {interval} seconds..."
            )
            time.sleep(interval)
            elapsed += interval
    except Exception as e:
    ...
    if len(nodes) == 0:
        raise Exception(
            f"No nodes found with the label {node_label_selector} after {timeout} seconds"
        )
