...
class CapacityReservationManager:
    ...
    def __init__(self, region: Optional[str] = None):
        ...
        if not self.region:
            raise ValueError(
                "AWS region is required. Set AWS_DEFAULT_REGION environment variable or provide region parameter."
            )

        try:
            self.ec2 = boto3.client("ec2", region_name=self.region)
            logger.info(
                "Successfully initialized Capacity Reservation Manager for region: %s",
                self.region,
            )
        ...
        **kwargs,
        ...
        try:
            ...
            if capacity_duration_hours:
                params["CapacityDurationHours"] = (
                    24 if capacity_duration_hours < 24 else capacity_duration_hours
                )
    ...
    def purchase_capacity_block(
        ...
        try:
            logger.info(
                "Purchasing capacity block with offering ID: %s",
    ...
    def get_capacity_reservation_summary(self, capacity_reservation_id: str) -> Dict:
        ...
        if not reservations:
            raise ValueError(
                f"Capacity reservation {capacity_reservation_id} not found"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="AWS Capacity Reservation Manager - Find and Purchase Capacity Blocks"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "-t",
        required=True,
        help="EC2 instance type (e.g., p5.48xlarge)",
    )
    parser.add_argument(
        "--instance-count",
        "-c",
    ...
    parser.add_argument(
        "--duration-hours", "-d", type=int, required=True, help="Duration in hours"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Only search for offerings, do not purchase",
    )
    ...
    try:
        ...
        if not offerings:
            logger.error(
                "No capacity block offerings found for the specified criteria."
            )
            sys.exit(1)
        ...
        logger.info(
            "Price: %s %s",
            best_offering.get("CurrencyCode", "USD"),
            best_offering.get("UpfrontFee"),
        )
        logger.info(
            "Duration: %s hours", best_offering.get("CapacityBlockDurationHours")
        )
        ...
        if args.dry_run:
            ...
            logger.info(
                "Total Instances: %s", capacity_reservation.get("TotalInstanceCount")
            )
            logger.info(
                "Availability Zone: %s", capacity_reservation.get("AvailabilityZone")
            )
