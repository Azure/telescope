...
class Node(KWOK):
    ...
    def validate(self):
        for i in range(self.node_count):
            ...
            try:
                ...
                raise RuntimeError(f"Validation failed for node {node_name}: {e}") from e
    ...
    def _validate_node_resources(self, node_name, node_info):
        ...
        if not allocatable or not capacity:
            raise RuntimeError(f"Node {node_name} is missing resource information (allocatable or capacity).")
...
if __name__ == "__main__":
    node_manager = Node(node_count=5)
    node_manager.create()
    node_manager.validate()
    node_manager.tear_down()
