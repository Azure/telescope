...
class NodePoolCRUD:
    ...
    def create_node_pool(self, node_pool_name, vm_size, node_count=1, gpu_node_pool=False):
        """
        ...
        """
        logger.info(f"Creating node pool '{node_pool_name}' with {node_count} nodes (GPU: {gpu_node_pool})")
    ...
    def scale_node_pool(self, node_pool_name, node_count,
                        progressive=False, step_size=1, wait_time=30, gpu_node_pool=False):
        """
        ...
        try:
            ...
            logger.info(f"{operation_type.replace('_', ' ').title()} node pool '{node_pool_name}' from {current_count} to {node_count} nodes")
            ...
            logger.info(f"Node pool '{node_pool_name}' {operation_type.replace('_', ' ')}d to {node_count} nodes successfully")
                        
            return result
        except Exception as e:
            logger.error(f"Failed to {operation_type.replace('_', ' ')} node pool '{node_pool_name}': {str(e)}")
            return False
    
    def _progressive_scale(self, node_pool_name, current_count, target_count, 
                          step_size=1, wait_time=30, operation_type="scale", gpu_node_pool=False):
        """
        ...
        """
        logger.info(f"Starting progressive scaling for node pool '{node_pool_name}' from {current_count} to {target_count} nodes (step size: {step_size})")
        ...
        for step in steps:
            logger.info(f"Scaling from {current_count if step == steps[0] else steps[steps.index(step)-1]} to {step} nodes")
        ...
        logger.info(f"Progressive scaling from {current_count} to {target_count} completed successfully")
        return result
    ...
    def delete_node_pool(self, node_pool_name):
        ...
        try:
            result = self.aks_client.delete_node_pool(
                node_pool_name=node_pool_name
            )
    ...
    def all(self, node_pool_name, vm_size=None, 
                            node_count=None, target_count=None, 
                            progressive=False, step_size=1, wait_time=30,
                            gpu_node_pool=False):
        """
        ...
        try:
            ...
            # 2. Scale up
            logger.info(f"Scaling up node pool '{node_pool_name}' to {target_count} nodes")
            ...
            logger.info(f"Scaling down node pool '{node_pool_name}' back to {node_count} nodes")
...
def main():
    ...
    common_parser.add_argument('--result-dir', default='.', help='Directory to save results')
    common_parser.add_argument('--kube-config', help='Path to kubeconfig file')
    common_parser.add_argument('--step-timeout', type=int, default=600, help='Wait time for each operation in seconds')
    common_parser.add_argument('--gpu-node-pool', action='store_true', help='Whether this is a GPU-enabled node pool')
    
    # Create command
    create_parser = subparsers.add_parser('create', parents=[common_parser], help='Create a node pool')
    ...
    create_parser.add_argument('--node-count', type=int, default=1, help='Number of nodes to create')
    create_parser.set_defaults(func=handle_node_pool_operation)
    
    # Scale command
    scale_parser = subparsers.add_parser('scale', parents=[common_parser], help='Scale a node pool')
    scale_parser.add_argument('--node-pool-name', required=True, help='Node pool name')
    scale_parser.add_argument('--target-count', type=int, required=True, help='Target node count')
    scale_parser.add_argument('--progressive', action='store_true', help='Scale progressively in steps')
    scale_parser.add_argument('--step-size', type=int, default=1, help='Number of nodes to add/remove in each step during progressive scaling')
    scale_parser.add_argument('--step-wait-time', type=int, default=30, help='Wait time in seconds between scaling steps')
    scale_parser.set_defaults(func=handle_node_pool_operation)
    
    # Delete command
    delete_parser = subparsers.add_parser('delete', parents=[common_parser], help='Delete a node pool')
    ...
    all_parser = subparsers.add_parser('all', parents=[common_parser], help='Run full lifecycle: create, scale-up, scale-down, delete a node pool')
    all_parser.add_argument('--node-pool-name', required=True, help='Node pool name')
    all_parser.add_argument('--vm-size', required=True, default='Standard_DS2_v2', help='VM size for node pool (for create operation)')
    all_parser.add_argument('--node-count', type=int, required=True, default=1, help='Initial number of nodes to create')
    all_parser.add_argument('--target-count', type=int, required=True, help='Target node count for scale-up operation')
    all_parser.add_argument('--progressive', action='store_true', help='Scale progressively in steps (for scaling operations)')
    all_parser.add_argument('--step-size', type=int, default=1, help='Number of nodes to add/remove in each step (for progressive scaling)')
    all_parser.add_argument('--step-wait-time', type=int, default=30, help='Wait time between scaling steps in seconds (for progressive scaling)')
