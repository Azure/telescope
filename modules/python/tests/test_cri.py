...
class TestCRIClusterLoaderFunctions(unittest.TestCase):
    ...
    def test_override_config_clusterloader2(self, mock_kubernetes_client, mock_open):
        ...
        handle.write.assert_any_call("CL2_NODE_PER_STEP: 2\n")
        ...
        handle.write.assert_any_call("CL2_PROMETHEUS_TOLERATE_MASTER: true\n")
        handle.write.assert_any_call("CL2_PROMETHEUS_CPU_SCALE_FACTOR: 30.0\n")
        handle.write.assert_any_call("CL2_PROMETHEUS_MEMORY_LIMIT_FACTOR: 30.0\n")
        handle.write.assert_any_call("CL2_PROMETHEUS_MEMORY_SCALE_FACTOR: 30.0\n")
    ...
    def test_override_command(self, mock_override):
        ...
        with patch.object(sys, 'argv', test_args):
            ...
            mock_override.assert_called_once_with(
                5, 1, 110, 3, "2m", "cpu", True, "10s", "aws", "test registry endpoint", "linux", False, False, "20s", False, "/tmp/override.yaml", None
            )
    ...
    def test_override_command_default_host_network(self, mock_override):
        ...
        with patch.object(sys, 'argv', test_args):
            main()
            mock_override.assert_called_once_with(
                5, 1, 110, 3, "2m", "cpu", True, "10s", "aws", "test registry endpoint", "linux", False, False, "15s", True, "/tmp/override.yaml", None
            )
    ...
    def test_override_command_with_memory_request_override(self, mock_override):
        ...
        with patch.object(sys, 'argv', test_args):
            main()
            mock_override.assert_called_once_with(
                1000, 1000, 7, 1, "60m", "memory", False, "10m", "aks", "acrperftestaue.azurecr-test.io", "linux", False, True, "30s", True, "/tmp/override.yaml", "1000Mi"
            )
    ...
    def test_execute_command(self, mock_execute):
        test_args = [
            "main.py", "execute", 
            "--cl2_image", "gcr.io/cl2:latest", 
            "--cl2_config_dir", "/configs", 
