...
| **AKS etcd server health/DB/leader** | ❌ | full `controlplane-etcd` metrics (minimal ingestion disabled) + aggregate CPU/memory/DB utilization from AKS platform metrics | persistent per-cluster AMW; optional reconstructed TSDB |
| **scheduler / controller-manager** | ❌ | full `controlplane-kube-scheduler` + `controlplane-kube-controller-manager` functional metrics | persistent per-cluster AMW; optional reconstructed TSDB |
...
| **ACNS network observability** | ❌ absent | native Cilium forward/drop count+byte metrics, Hubble DNS/drop/TCP/flow metrics from a real-node probe, and filtered host-local Container Network Logs | native TSDB + per-node CNL archives |

Every future CL2 run writes `telemetry/telemetry-audit-self-hosted.{json,md}` before snapshot/teardown. The pipeline artifact carries them beside the TSDB tarball; blob storage keeps them under a separate `telemetry-audit-self-hosted/` prefix so the snapshot-folder invariant (**blob-count = cluster-count**) stays true. The opt-in AKS control-plane canary also writes `telemetry-audit-managed.{json,md}`, per-workspace capacity audits, and a schema-v2 run manifest containing every cluster-to-AMW mapping, query window, enabled targets, cluster ARM IDs, and run-unique alphanumeric Prometheus aliases (`<run_id>_<role>`, punctuation normalized to `_`). Missing required families remain explicit warnings so workload results are preserved; actual AMW `LimitThrottling` drops are a hard completeness failure.

**Full-telemetry canary 73635.** Both native snapshots loaded successfully with `run/build/tier/snapshot_cluster` on all 172 827 / 173 739 series. Each cluster had 3/3 real-node kubelet and cAdvisor targets up, 103 KSM nodes, 202 Pods, and 204 real container CPU/memory series. The 100 mock agents consumed 0.828 / 0.733 cores and 6.92 GB working-set memory per cluster (about 8.3 / 7.3 mCPU and 69 MB per agent). The managed reconstruction exported 1 764 discovered metric names, 63.48 million retained samples, zero metric-query errors, and only 5 011 timestamp fallbacks (0.0079%), but it was **not sample-complete**: the shared AMW had 1 million active-series and 1 million events/minute limits, while the duplicated full scrape reached 1 265 508 active series and about 3.9 million events/minute. Azure reported `LimitThrottling` from 13:58Z onward, including 441 569 094 dropped events and 20 197 041 dropped time-series samples during the build. API server and etcd platform usage was present for both clusters (2% / 3% API-server CPU/memory and 3% / 1% etcd CPU/memory).

The canary exposed three wiring gaps and one capacity-design gap: exporter ClusterRole objects were incorrectly submitted through the `monitoring` namespace, shell gates saw matrix `mock_mode` as `True` rather than normalized `true`, the log query fixed its end timestamp before delayed LAW rows arrived, and managed Prometheus duplicated the native dataplane collection into a quota-limited persistent workspace. The follow-up wiring separates cluster-scoped RBAC, normalizes mock mode for KWOK scrape/audit gates, records a later log window independently from the managed-metric reconstruction window, and restricts AMW to AKS control-plane targets plus the small API-server backend exporter and collector-health job.

**Control-plane-only canary 73704.** Both native telemetry audits passed, and AMW contained only the intended `controlplane-apiserver`, `controlplane-etcd`, `controlplane-kube-scheduler`, `controlplane-kube-controller-manager`, collector-health, and API-backend-exporter jobs. The managed coverage audit passed for both clusters. The workspace reached 49.45% active-series utilization and 134.60% of the nominal events/minute metric, but Azure reported zero `LimitThrottling` drops; 1 203 discarded samples were duplicates only. The original pipeline guard incorrectly treated nominal utilization above 100% as data loss and skipped reconstruction. Recovery from raw AMW produced a 50 MB snapshot with 543 metrics, 18.87 million samples, zero query errors, and zero timestamp fallbacks. Because the false-positive stop froze the platform window early, aggregate API-server/etcd CPU-memory and delayed LAW logs were not captured in that run's artifact; one clean end-to-end canary is still required.

The managed path is intentionally separate from the native TSDB snapshot: AKS control-plane endpoints cannot be scraped reliably by self-hosted Prometheus, and AMW has no `/admin/tsdb/snapshot` or `remote_read`. Native Prometheus remains authoritative for Cilium, Hubble, ClusterMesh, KWOK, KSM, kubelet, and cAdvisor; each cluster receives its own persistent AMW containing only managed control-plane series, collector health, and the API-server backend exporter. The main scale pipeline retains raw AMW data and the complete run manifest; optional fixed-step TSDB reconstruction runs later on a higher-capacity devbox. Reconstructed tarballs are queryable but not lossless: exact scrape timestamps, staleness markers, exemplars, and native histograms cannot be recovered.
...
- **Telemetry gap — AKS-managed control plane.** Self-hosted Prometheus cannot reliably scrape the hidden AKS control-plane replicas. The rollout now assigns one persistent AMW per AKS cluster, creates missing workspaces in server-side ARM batches, enables all supported control-plane targets with minimal ingestion disabled, and leaves duplicated dataplane targets off. Per-workspace preflight and run-window audits use actual `LimitThrottling` drops as the hard completeness signal. Azure platform metrics are exported before cluster deletion; AMW reconstruction, LAW export, and AKSInfra export are deferred.
...
| AKS managed control-plane metrics | ✅ build 73704: intended jobs only, both-cluster managed audit complete, zero limit drops |
| reconstructed managed TSDB export | ✅ optional devbox recovery proven on build 73704: 543 metrics / 18.87 M samples / 50 MB, zero query errors/fallbacks |
...
**AKS control-plane metrics (new-run canary).** The n=2 mock smoke is the opt-in rollout stage. It registers the subscription preview feature, creates/reuses one AMW per cluster (`cmsh-scale-eastus2euap-amw-<role>`) in persistent RG `clustermesh-scale-prom-snapshots`, and attaches each AKS cluster to its assigned workspace. Missing AMWs are created in server-side ARM batches of ten. The workspaces are deliberately outside the ephemeral run RG, so raw Prometheus data survives cleanup. Audit artifacts are uploaded beside the snapshots at:
...
clustermesh-scale-2/managed-control-plane/<run_id>/
  ...
  workspace-<role>/amw-capacity.json
  workspace-<role>/amw-capacity-summary.json
  workspace-<role>/amw-capacity-summary.md
```
