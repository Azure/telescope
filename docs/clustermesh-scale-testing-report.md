...
- **Telemetry gap — AKS-managed control plane.** Self-hosted Prometheus cannot reliably scrape the hidden AKS control-plane replicas. An opt-in n=2 canary attaches every run cluster to one persistent AMW, enables all supported control-plane targets with minimal ingestion disabled, leaves duplicated cluster/dataplane targets off, exports all available AKS platform metrics, and builds a queryable reconstructed TSDB tarball. A 50% preflight headroom gate and run-window drop audit prevent a quota-truncated export from being marked complete. The earlier full-duplication canary exceeded both workspace limits; the control-plane-only pipeline canary still requires validation.
...
| AKS managed control-plane metrics | ⚠️ forwarding proven, but build 73635 was quota-throttled; control-plane-only pipeline canary pending |
| reconstructed managed TSDB export | ⚠️ reconstruction mechanics passed, but the 73635 source window was quota-truncated; clean control-plane-only canary pending |
