...
Every future CL2 run writes `telemetry/telemetry-audit-self-hosted.{json,md}` before snapshot/teardown. The pipeline artifact carries them beside the TSDB tarball; blob storage keeps them under a separate `telemetry-audit-self-hosted/` prefix so the snapshot-folder invariant (**blob-count = cluster-count**) stays true. The opt-in AKS control-plane canary also writes `telemetry-audit-managed.{json,md}`, `amw-capacity-summary.{json,md}`, and a run manifest containing the AMW, query window, enabled targets, cluster ARM IDs, and run-unique alphanumeric Prometheus cluster aliases (`<run_id>_<role>`, punctuation normalized to `_`). Missing required families remain explicit warnings so workload results are preserved; AMW limit throttling is a hard completeness failure and blocks TSDB reconstruction.
...
The managed path is intentionally separate from the native TSDB snapshot: AKS control-plane endpoints cannot be scraped reliably by self-hosted Prometheus, and AMW has no `/admin/tsdb/snapshot` or `remote_read`. Native Prometheus remains authoritative for Cilium, Hubble, ClusterMesh, KWOK, KSM, kubelet, and cAdvisor; AMW disables those default/annotation/custom dataplane scrapes and retains only managed control-plane series, collector health, and the API-server backend exporter. For offline use, the pipeline enumerates every run-scoped metric name, exports fixed-step PromQL matrices, merges all available AKS platform metrics, and backfills native Prometheus blocks with `promtool`. This reconstructed tarball is queryable but not lossless: exact scrape timestamps, staleness markers, exemplars, and native histograms cannot be recovered.
...
- **Telemetry gap — AKS-managed control plane.** Self-hosted Prometheus cannot reliably scrape the hidden AKS control-plane replicas. An opt-in n=2 canary attaches every run cluster to one persistent AMW, enables all supported control-plane targets with minimal ingestion disabled, leaves duplicated cluster/dataplane targets off, exports all available AKS platform metrics, and builds a queryable reconstructed TSDB tarball. A 40% preflight headroom gate prevents active-series overlap between runs; the run-window audit uses actual `LimitThrottling` drops as the hard completeness signal. The earlier full-duplication canary exceeded both workspace limits; the control-plane-only collection and offline reconstruction are proven, but one clean end-to-end pipeline canary is still required.
...
| reconstructed managed TSDB export | 🧪 build 73704 recovered locally: 543 metrics / 18.87 M samples / 50 MB, zero query errors/fallbacks; clean in-pipeline reconstruction pending |
