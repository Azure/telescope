...
**Full-telemetry canary 73635.** Both native snapshots loaded successfully with `run/build/tier/snapshot_cluster` on all 172 827 / 173 739 series. Each cluster had 3/3 real-node kubelet and cAdvisor targets up, 103 KSM nodes, 202 Pods, and 204 real container CPU/memory series. The 100 mock agents consumed 0.828 / 0.733 cores and 6.92 GB working-set memory per cluster (about 8.3 / 7.3 mCPU and 69 MB per agent). The managed reconstruction exported all 1 764 discovered metric names, 63.48 million samples, zero metric-query errors, and only 5 011 timestamp fallbacks (0.0079%). API server and etcd platform usage was present for both clusters (2% / 3% API-server CPU/memory and 3% / 1% etcd CPU/memory).

The canary exposed three wiring gaps rather than data-source gaps: exporter ClusterRole objects were incorrectly submitted through the `monitoring` namespace, shell gates saw matrix `mock_mode` as `True` rather than normalized `true`, and the log query fixed its end timestamp before delayed LAW rows arrived. The follow-up wiring separates cluster-scoped RBAC, normalizes mock mode for KWOK scrape/audit gates, and records a later log window independently from the managed-metric reconstruction window.
