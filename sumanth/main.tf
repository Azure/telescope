...
resource "azurerm_storage_blob" "blob" {
  ...
  type                   = "Block"
  source_content         = ".create table SumanthTest (Level:string, Timestamp:datetime, UserId:string, TraceId:string, Message:string, ProcessId:int32)"
}
...
resource "azurerm_kusto_eventgrid_data_connection" "evengrid_connection" {
  ...
  mapping_rule_name            = "mapping-rule"
