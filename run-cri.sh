...
show_help() {
    ...
    echo "Options:"
    echo "  --setup-cluster      Set up the Kubernetes cluster using Terraform and Azure CLI."
    echo "  --teardown-cluster   Tear down the Kubernetes cluster and resources."
    echo "  --test-cri           Run the CRI resource consume test scenario."
...
main() {
    ...
    if [[ "$setup_cluster_flag" == "1" ]]; then
        setup_cluster
    fi
    if [[ "$teardown_cluster_flag" == "1" ]]; then
        teardown_cluster
    fi
